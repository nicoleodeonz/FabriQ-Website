...
function extractUploadRelativePath(value) {
...
function resolveLocalAssetPath(value) {
  const relative = extractUploadRelativePath(value);
  if (!relative) return null;
  const candidate = path.resolve(uploadsDir, relative);
  if (!candidate.startsWith(uploadsDir)) return null;
  return candidate;
}
...
async function migrateProductModels() {
  ...
  try {
    ...
    for (const product of products) {
      const current = String(product.model3dUrl || '').trim();
      ...
      if (isExternalNonLocalUrl(current)) {
        skippedExternal += 1;
        continue;
      }

      const localPath = resolveLocalAssetPath(current);
      if (!localPath) {
        unsupportedFormat += 1;
        continue;
      }
      ...
      if (!fs.existsSync(localPath)) {
        missingLocalFile += 1;
        failures.push({ id: String(product._id), name: product.name, reason: `Missing local file: ${localPath}` });
        continue;
      }

      try {
        const stored = await storeAssetFromLocalPath(localPath, { folder: 'products/models', resourceType: 'raw' });
        product.model3dUrl = stored.url;
        await product.save();
        updated += 1;
      } catch (error) {
        failures.push({ id: String(product._id), name: product.name, reason: error instanceof Error ? error.message : 'Unknown upload failure' });
      }
    }
